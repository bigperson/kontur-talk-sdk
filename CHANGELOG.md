...
### Изменено
- Таймауты по умолчанию для исходящих HTTP-запросов к API Контур.Толка (`connect_timeout = 5s`, `timeout = 20s`) в конструкторе `TalkClient`, предотвращающие бесконечное зависание воркеров при недоступности эндпоинтов.
